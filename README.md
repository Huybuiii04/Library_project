...
| Layer | Tables | Records | Processing Time | Output Size |
|-------|--------|---------|-----------------|-------------|
| **Bronze** | 4 | 1,072,089 | ~3-5 minutes | ~2.5 GB |
| **Silver** | 4 | 1,072,089 | ~2-3 minutes | ~2.3 GB |
| **Gold** | 6 (4 dim + 2 fact) | 1,072,101 | ~2-4 minutes | ~2.1 GB |

### Real-Time CDC Performance

| Metric | Value | Notes |
|--------|-------|-------|
| **CDC Latency** | <2 seconds | MySQL → order_final |
| **Throughput** | ~5,000 events/sec | With 5 workers (2+3) |
| **Duplicate Rate** | 0% | Guaranteed by atomic locks |
| **Snapshot Time** | ~30 seconds | 72,089 records (25K orders + 47K details) |
| **Consumer Lag** | 0 | Steady state after processing |

### Resource Utilization

| Container | CPU | Memory | Disk I/O |
|-----------|-----|--------|----------|
| **spark-master** | 2 cores | 4 GB | Medium |
| **spark-worker-1** | 2 cores | 4 GB | High |
| **spark-worker-2** | 2 cores | 4 GB | High |
| **kafka-broker** | 1 core | 2 GB | Medium |
| **mysql-db** | 1 core | 2 GB | Low |
| **minio-storage** | 1 core | 1 GB | High |
| **redis-cache** | 0.5 core | 512 MB | Low |
...
1. **Medallion Architecture**: Bronze → Silver → Gold for clear data quality progression
2. **Incremental Loading**: Use timestamp-based partitioning for efficient updates
3. **Deduplication**: Window functions with `row_number()` for handling duplicates
4. **Star Schema**: Denormalized dimensions and facts for query performance
5. **Partitioning**: Year/month/day partitions for efficient time-based queries

### Real-Time CDC Best Practices

1. **Idempotency**: Use Redis atomic locks (`SET NX`) for exactly-once semantics
2. **Consumer Groups**: Multiple workers in same group for load balancing
3. **Caching Strategy**: Short TTL (120s) for order cache to handle late arrivals
4. **Error Handling**: Graceful degradation with comprehensive logging
5. **Monitoring**: Track consumer lag and processing throughput

### Production Considerations

1. **Scalability**: Add more Spark workers or Kafka consumers as needed
2. **High Availability**: Configure Kafka with replication factor ≥ 3
3. **Disaster Recovery**: Regular MinIO backups and Kafka topic retention
4. **Security**: Enable authentication and encryption for production
...
- [Change Data Capture with Debezium](https://debezium.io/documentation/reference/tutorial.html)
...
- [MinIO Console](http://localhost:9000) - Browse object storage
- [Kafka UI (optional)](https://github.com/provectus/kafka-ui) - Visualize Kafka topics
...
4. Test thoroughly (batch ETL and real-time CDC)
...
- Debezium team for CDC framework
...
- **Documentation**: Check `init-scripts/run.txt` for complete command reference
