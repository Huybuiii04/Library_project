...
def check_and_trigger(order_id, producer):
    """Check if order is complete and trigger processing."""
...
def process_message(message, producer):
    """Process order message from Kafka."""
    try:
        order_payload = message.value.get("payload", {})
        if "after" not in order_payload:
            logger.warning(f"Invalid message structure: {message.value}")
            return
        
        order_data = order_payload["after"]
        ...
        redis_dynamic.expire(f"order_info:{order_id}", 120)  # 2 minutes TTL
        logger.info(f"[ORDER_CACHED] {order_id} - customer: {customer_id}, products: {num_products}")
        
        # Note: do NOT call check_and_trigger here to avoid duplicates
        # Triggering will be done by order_details.py only
    except Exception as e:
        logger.error(f"Error processing message: {e}", exc_info=True)

def order_info_worker(worker_id: int):
    """Worker process to consume order messages."""
    logger.info(f"Starting order_info_worker {worker_id}")
    ...
    try:
        logger.info(f"Worker {worker_id} consuming from mysql.project_db.orders")
        while True:
...
def main():
    ...
    logger.info(f"Starting {num_workers} order tracking workers...")
